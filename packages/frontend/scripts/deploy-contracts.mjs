...
import { deployUnirep, deployVerifierHelper } from '@unirep/contracts/deploy/index.js'
...
async function deployUnirepSocial(deployer, UnirepAddr, helperAddr, epochLength) {
  const verifierFactory = new ContractFactory(DATA_PROOF_VIRIFIER.abi, DATA_PROOF_VIRIFIER.bytecode, deployer)
  const verifierContract = await verifierFactory.deploy()
  await verifierContract.deployTransaction.wait()

  const appFactory = new ContractFactory(UNIREP_APP.abi, UNIREP_APP.bytecode, deployer)
