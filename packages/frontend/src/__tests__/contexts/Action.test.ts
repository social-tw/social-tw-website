import {
  ActionStatus, ActionType, addAction, failActionById, removeActionByCommentId,
  removeActionById, succeedActionById, useActionStore
...
describe('useActionStore', () => {
  ...
  it('add a post action', () => {
    ...
    expect(result.current.entities[actionId]).toHaveProperty('type', ActionType.Post);
    expect(result.current.entities[actionId]).toHaveProperty('status', ActionStatus.Pending);
    expect(result.current.entities[actionId]).toHaveProperty('submittedAt');
    expect(result.current.entities[actionId]).toHaveProperty('data', postData);
  ...
  it('add a comment action', () => {
    ...
    expect(result.current.entities).toHaveProperty(actionId);
    expect(result.current.entities[actionId]).toHaveProperty('id');
    expect(result.current.entities[actionId]).toHaveProperty('type', ActionType.Comment);
    expect(result.current.entities[actionId]).toHaveProperty('status', ActionStatus.Pending);
    expect(result.current.entities[actionId]).toHaveProperty('submittedAt');
    expect(result.current.entities[actionId]).toHaveProperty('data', commentData);
    expect(result.current.list).toContain(actionId);
    expect(result.current.latestId).toBe(actionId);
  })
  ...
  it('succeed an action by id', () => {
    ...
    expect(result.current.entities[actionId]).toHaveProperty('status', ActionStatus.Success);
  })
  ...
  it('fail an action by id', () => {
    ...
    expect(result.current.entities[actionId]).toHaveProperty('status', ActionStatus.Failure);
  })
