...
export interface NotificationConfig {
    type: string
    message: string
    actions?: NotificationAction[] | ((targetId: string) => NotificationAction[])
    icon?: React.FunctionComponent<React.SVGProps<SVGSVGElement>>
}

// Notification action handlers
const goToPostPage = (postId: string) => () => console.log("Navigating to post page with ID: ${postId}")
const openCheckInDialog = () => console.log("Opening check-in dialog")
const openReportDialog = () => console.log("Opening report dialog")
const openAdjudicationDialog = () => console.log("Opening adjudication dialog")

export const notificationConfig: Record<string, NotificationConfig> = {
    SIGN_UP_SUCCESS: {
        type: "SIGN_UP_SUCCESS",
        message: "恭喜註冊成功，歡迎加入Unirep Social TW。有任何疑問可以參閱「平台說明」頁面",
    ...
    POST_SUCCEEDED: {
        ...
        actions: (targetId: string) => [
            ...
            { label: "前往查看", execute: () => goToPostPage(targetId) }
        ],
    ...
    POST_FAILED: {
        ...
        actions: [{ label: "前往查看", execute: goToPostPage("123") }],
        icon: PostFailedIcon,
    },
    // COMMENT_SUCCEEDED: {
    ...
    // REPORT_FAILED: {
    //     type: "REPORT_FAILED",
    //     message: "你的不當內容檢舉申請送出未成功，可透過連結前往查看，並再嘗試送出申請一次",
    //     actions: [{ label: "前往查看", execute: openReportDialog }],
    ...
    // BE_REPORTED: {
    //     type: "BE_REPORTED",
    //     message: "很遺憾通知你，你的內容（貼文/留言）被檢舉，並且評判通過，你的聲譽分數降低 5 分",
    //     actions: [{ label: "前往查看", execute: goToPostPage(123) }],
    //     icon: ReportedIcon,
    // },
    // ADJUDICATE_SUCCEEDED: {
    //     type: "ADJUDICATE_SUCCEEDED",
    //     message: "你的檢舉案件評判協助完成，你的聲譽分數提高 1 分，評判最終結果出爐會再另行通知",
    //     actions: [{ label: "前往查看", execute: goToPostPage(123) }],
    //     icon: AdjudicateSuccessIcon,
    // },
    // ADJUDICATE_FAILED: {
    //     type: "ADJUDICATE_FAILED",
    //     message: "你的檢舉案件評判協助未成功，可透過連結前往查看，並再嘗試評判一次",
    //     actions: [{ label: "前往查看", execute: openAdjudicationDialog }],
    ...
    //     message: "有新的檢舉案被提交，需要 5 位用戶參與檢舉案評判，參與評判者可提高聲譽分數 1 分",
    //     actions: [{ label: "前往查看", execute: openAdjudicationDialog }],
    //     icon: NewAdjudicationIcon,
    // },
    // LOW_REPUTATION: {
    //     type: "LOW_REPUTATION",
    //     message: "你的聲譽分數目前低於 0 分，可以透過進行「每日簽到」來提高聲譽分數，進而恢復你的平台操作權利。點擊連結前往每日簽到",
    //     actions: [{ label: "前往查看", execute: openCheckInDialog }],
