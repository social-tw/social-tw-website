...
export function useNotificationConfig(): Record<NotificationType, NotificationMeta> {
    const navigate = useNavigate()
    const { setAdjuducateDialogOpen } = useAdjudicateStore()
    const { toggleCheckIn } = useCheckInStore()
    const { setIsOpen } = useDialog()

    const viewPost = useCallback((data: NotificationData) => {
        navigate(`/posts/${data.link}`)
    }, [navigate])

    const rewritePost = useCallback((data: NotificationData) => {
        navigate(`/?failedPostId=${data.link}`)
    }, [navigate])

    const viewComment = useCallback((data: NotificationData) => {
        navigate(`/posts/${data.link}`)
    }, [navigate])

    const rewriteComment = useCallback((data: NotificationData) => {
        navigate(`/posts/${data.link}`)
    }, [navigate])

    const reportResult = useCallback((data: NotificationData) => {
        navigate(`/reports/${data.link}`)
    }, [navigate])

    const reportDialog = useCallback((data: NotificationData) => {
        navigate(`${data.link}`)
        setIsOpen(true)
    }, [navigate, setIsOpen])
