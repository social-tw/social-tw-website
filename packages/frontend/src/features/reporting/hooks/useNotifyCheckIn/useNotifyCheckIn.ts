...
export function useNotifyCheckIn() {
    ...
    const [discardedAt, saveDiscardedAt] = useLocalStorage<string | null>(DISCARDED_AT, null)

    const isOpen = useMemo(() => !!score && score < 0 && !discardedAt, [score, discardedAt])
    ...
    useEffect(() => {
        if (
            !discardedAt ||
            dayjs().isSame(dayjs(discardedAt), 'day')
        ) {
            return
        }
