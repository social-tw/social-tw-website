...
export type Action = BaseAction<ActionType.Post, PostData> | BaseAction<ActionType.Comment, CommentData>;
...
function partializeStorage(state: ActionState): ActionState {
    ...
    return {
        entities: Object.fromEntries(failedIds.map((id) => [id, state.entities[id]])),
        list: failedIds
    };
}
...
export function commentActionsSelector(state: ActionState) {
    return Object.values(state.entities)
        .filter((action) => action.type === ActionType.Comment);
}
...
export function countByTimeRangeSelector(startTime: number, endTime: number) {
    return function (state: ActionState) {
        return Object.values(state.entities)
            .filter((action) => action.submittedAt > startTime && action.submittedAt <= endTime)
            .length;
    }
}

export function createAction(type: ActionType, data: PostData | CommentData): Action {
    return {
        id: nanoid(),
        type,
...
export function succeedActionById(id: string, data: Partial<PostData | CommentData> = {}) {
    useActionStore.setState((state) => {
...
export function failActionById(id: string, data: Partial<PostData | CommentData> = {}) {
    useActionStore.setState((state) => {
